...
async function fetchArticles(): Promise<Article[]> {
  try {
    ...
    const [regular, devMessages, funding] = await Promise.all([
      fetch(
        `${SUPABASE_URL}/rest/v1/articles?select=id,title_ai,summary,source,country,pub_date,link,section,created_at&section=neq.NOT_RELEVANT&section=neq.Dev_Messages&section=neq.Funding_News&created_at=gte.${since}&order=created_at.desc&limit=300`,
      ...
      fetch(
        `${SUPABASE_URL}/rest/v1/articles?select=id,title_ai,summary,source,country,pub_date,link,section,created_at&section=eq.Funding_News&order=pub_date.desc&limit=1000`,
        { headers, next: { revalidate: 900 } }
      ).then(r => r.json()),
    ])
    return [...regular, ...devMessages, ...funding]
  } catch {
    return []
  }
}
